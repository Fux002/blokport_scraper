"""
Varsha Stones Inventory Scraper (Authenticated)
================================================
Same SlabWare SaaS platform as Ferraz/Bruma - this tenant has the public
viewer disabled, so we authenticate by reading the cookies directly from
your local Chrome browser. As long as you're logged into Varsha in Chrome
on this Mac, the scraper inherits that session.

Key differences from the Ferraz scraper:
  - Auth: reads cookies from Chrome via browser_cookie3 at startup
  - Detail endpoint: 'DetalheBundleNovo' (not 'DetalheBundle' - SlabWare
    upgraded the endpoint for this tenant)
  - Validates auth before scraping: hits an endpoint that requires login
    and bails fast if the response shape says we're not authenticated

Install once:
    pip3 install curl_cffi browser_cookie3 --break-system-packages

When cookies expire (typically a few hours up to a day), just visit
varshastones.slabware.com in Chrome to refresh the session, then re-run.
"""
...
OUTPUT_CSV = SCRIPT_DIR / f"varsha_products_{TIMESTAMP_TAG}.csv"
LOG_FILE = SCRIPT_DIR / f"varsha_scraper_{TIMESTAMP_TAG}.log"
FAILURES_CSV = SCRIPT_DIR / f"varsha_failures_{TIMESTAMP_TAG}.csv"
...
S_TOKEN = ("9t5AptkpH/x2GD3oStI0wQzByO78puNBq5ZOBU2x8a1DsWdZJ9W+UFgwpAvr7qTXgoGcUrL"
           "nmVFKc1f53xV19kVT86meSR82TYzwo9Agi4tIXRQahJ4warnLVR8jPRR5GvDMZHS150+GTMH"
           "8nGEiPnRZLEL84a9WDfjUy1sjQtzyLmYfuQwNE/PhNf/i7sbVmXWD8yPXXlKACcextbfJdEj"
           "kzHToZS9V+RymVyhLsMDnHUOVinQ0TzRCs6+uRB/9FOamLYDUfY4LtO7ehKj8ZaPV65CLEru"
           "0ElwwmnOyQaNwsMfwD9TYrbOAyvxyDRQTjgTS/QBmCLCT1fwGRynKcZI+0nyXkdOc6McVLpF"
           "ApMZFKbkYe2MRmJ6ggsZ41ED1viRst30BZniPQV4V10x9kVIvnW776daNGqkKjcSoWTs7NJS"
           "rYbO5rCfTgW64Sp5MwPaSbN2387ySzreRGJUFcIW2KSaa1kzqlujRx1dfqHM=")
...
# Note: this tenant uses 'DetalheBundleNovo' (the upgraded endpoint), not the
# older 'DetalheBundle' that Ferraz/Bruma use. Confirmed via Network tab.
DETAIL_API_URL = f"{BASE}/FullInventory.aspx/DetalheBundleNovo"
...
logger = logging.getLogger('varsha')
...
def load_cookies_from_chrome():
    """Read varshastones.slabware.com cookies from local Chrome.

    Uses browser_cookie3, which on macOS reads Chrome's encrypted cookie
    database directly. The first time the script runs you'll get a
    macOS keychain prompt asking to allow Python access - allow it.
    """
    try:
        import browser_cookie3
    except ImportError:
        raise RuntimeError(
            "browser_cookie3 not installed. Run:\n"
            "  pip3 install browser_cookie3 --break-system-packages")

    logger.info("Loading cookies from local Chrome browser...")
    try:
        # domain_name filters to just varshastones.slabware.com cookies
        cj = browser_cookie3.chrome(domain_name='varshastones.slabware.com')
    except Exception as e:
        raise RuntimeError(
            f"Failed to read Chrome cookies: {e}\n"
            "Make sure: (1) Chrome is installed, (2) you're logged into "
            "varshastones.slabware.com in Chrome, (3) macOS allowed Python "
            "to access the keychain when prompted.")

    cookies_added = 0
    for cookie in cj:
        # curl_cffi session.cookies accepts standard cookielib cookies
    ...
    if cookies_added == 0:
        raise RuntimeError(
            "No cookies found for varshastones.slabware.com in Chrome. "
            "Open the site in Chrome, log in, then re-run this script.")

    logger.info("Loaded %d cookies from Chrome", cookies_added)
    cookie_names = sorted(c.name for c in cj)
    logger.info("Cookie names present: %s", cookie_names)

    # Sanity check the auth-bearing cookies
    must_have = ['ASP.NET_SessionId']
    missing = [c for c in must_have if c not in cookie_names]
    if missing:
        raise RuntimeError(
            f"Missing required auth cookies: {missing}. "
            "Log into Varsha in Chrome and try again.")
    if 'DistGeral' not in cookie_names:
        logger.warning("DistGeral cookie not found - you may not be authenticated. "
                       "If the listing call fails, log into Varsha in Chrome.")
    if 'cf_clearance' not in cookie_names:
        logger.warning("cf_clearance cookie not found - Cloudflare may block. "
                       "Visit Varsha in Chrome to refresh CF clearance.")


def warm_up_session():
    """GET the inventory page once to seed any session state. With cookies
    already loaded, this should land on the actual inventory page (status
    200) rather than redirecting to /AccessSecurity.aspx (status 302).
    """
    logger.info("Warming up session with GET %s ...", PAGE_URL[:80] + "...")
    r = session.get(PAGE_URL, timeout=30, allow_redirects=False)
    logger.info("  Status: %d, cookies in session: %d",
                r.status_code, len(session.cookies))

    if r.status_code == 302:
        loc = r.headers.get('location', '')
        if 'AccessSecurity' in loc or 'login' in loc.lower():
            raise RuntimeError(
                f"Server redirected to {loc} - your session is not "
                "authenticated. Log into varshastones.slabware.com in "
                "Chrome and re-run this script.")
        raise RuntimeError(f"Unexpected 302 redirect to {loc}")

    if r.status_code != 200:
        raise RuntimeError(
            f"Warm-up GET failed: {r.status_code}. Response: {r.text[:300]}")


def verify_auth():
    """Make a real authenticated API call to confirm cookies actually work.

    Hitting the listing endpoint is the cleanest verification - if we're
    not authenticated, SlabWare typically returns either an HTML login
    page or an empty bundles array.
    """
    logger.info("Verifying authentication with a probe API call...")
    payload = {"inicio": 0, "json": ""}
    r = session.post(API_URL, headers=API_HEADERS,
                     data=json.dumps(payload), timeout=30)

    if r.status_code != 200:
        raise RuntimeError(
            f"Auth probe failed: HTTP {r.status_code}. "
            f"Response: {r.text[:300]}\n"
            "Likely your session has expired - log in again in Chrome.")

    try:
        j = r.json()
    except Exception:
        # Most common failure mode: server returned an HTML login page
        if '<html' in r.text.lower() or 'login' in r.text.lower():
            raise RuntimeError(
                "Auth probe returned HTML (likely a login page). "
                "Your session is not authenticated - log into Varsha in "
                "Chrome and re-run.")
        raise RuntimeError(f"Auth probe returned non-JSON: {r.text[:200]}")

    d = j.get('d') or {}
    bundles_str = d.get('Bundles', '[]')
    try:
        bundles = json.loads(bundles_str) if isinstance(bundles_str, str) else bundles_str
    except Exception:
        bundles = []

    if not bundles:
        raise RuntimeError(
            "Auth probe returned 0 bundles. Either your session is not "
            "authenticated, or the inventory is genuinely empty. Verify by "
            "visiting the inventory page in Chrome.")

    permissoes = d.get('Permissoes') or {}
    logger.info("  Auth OK. First batch returned %d bundles.", len(bundles))
    logger.info("  Permissoes.Logado: %s", permissoes.get('Logado'))
    logger.info("  Permissoes.Preco: %s", permissoes.get('Preco'))
    logger.info("  Permissoes.MostrarPrecoDeslogado: %s",
                permissoes.get('MostrarPrecoDeslogado'))
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("VARSHA STONES INVENTORY SCRAPER (Authenticated)")
    logger.info("=" * 70)
    ...
    # Authentication: read cookies from Chrome, then verify they work
    load_cookies_from_chrome()
    warm_up_session()
    verify_auth()
