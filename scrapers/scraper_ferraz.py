"""
Ferraz Brasil Inventory Scraper
================================
Hits the SlabWare WebMethod /FullInventory.aspx/ObterListaBundles directly.
Outputs one CSV row per bundle and downloads each bundle's primary photo
to ./images/ as {bundle_id}.{ext}.

The whole site sits behind Cloudflare's bot challenge, so we use curl_cffi
to impersonate Chrome's TLS fingerprint. A warm-up GET seeds session cookies
before any POST calls.
...
OUTPUT_CSV = SCRIPT_DIR / f"ferraz_products_{TIMESTAMP_TAG}.csv"
LOG_FILE = SCRIPT_DIR / f"ferraz_scraper_{TIMESTAMP_TAG}.log"
FAILURES_CSV = SCRIPT_DIR / f"ferraz_failures_{TIMESTAMP_TAG}.csv"
...
logger = logging.getLogger('ferraz')
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("FERRAZ BRASIL INVENTORY SCRAPER")
    logger.info("=" * 70)
