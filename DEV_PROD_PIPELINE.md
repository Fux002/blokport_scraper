...
by `SCRAPER_ENV` (`development` default, `production`):
...
So you **scrape once**, then run `catalog`/`tree` under each `SCRAPER_ENV` to produce
...
|---|---|---|
| `SCRAPER_ENV` | `development` | `production` flips the S3 path segment to `prod/` and enables guards |
| `SCRAPER_S3_BUCKET` | dev bucket | The bucket; **must be set in prod** (a prod run on the dev bucket warns) |
| `SCRAPER_S3_REGION` | `eu-west-1` | AWS region |
| `SCRAPER_AWS_PROFILE` | `default` | Credentials profile (use the task IAM role on AWS) |
| `SCRAPER_S3_DRY_RUN` | `true` in development, `false` in production | the ECS task sets `false`; a laptop never writes the bucket by default |
| `SCRAPER_IMAGE_MODE` | `passthrough` | `s3` to download → process → re-host product photos |
| `SCRAPER_IMAGE_PROCESSING` | `false` | `true` to enhance/de-watermark before re-host |
| `SCRAPER_KEEP_SCRAPED` | `false` | `true` to also keep the raw download in the sibling `scraped/` folder |
| `SCRAPER_VARIANT_IMAGE_BASE` | dev variations URL | The variant `{Key}.png` base stamped into the catalog |
...
- `{env}/products/scraped/<src_site>/<sha256>.jpg` — the raw download, same filename
  (only when `SCRAPER_KEEP_SCRAPED=true`; downloads are otherwise in-memory and the
...
- The S3 bucket base in image URLs — the env var `SCRAPER_VARIANT_IMAGE_BASE`
...
1. Build the prod variant file: set the prod values and re-run the catalog —
   ```bash
   SCRAPER_VARIANT_IMAGE_BASE="https://<prod-bucket>/.../variations/" \
