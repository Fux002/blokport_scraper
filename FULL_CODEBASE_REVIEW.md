...
- **Only the ingest gate aborts on FAILED** -- `run.py:371`. `gates/__init__` documents "a FAILED gate aborts before emit," but clean/process gate FAILED status is recorded and discarded. A whole-batch origin regression -> PROCESS gate FAILED, yet the run emits the ~10% survivors and reports completed. _RESOLVED: by design; doc reconciled to code. Clean/process are ADVISORY: per-row `RejectReason`/`is_emittable` is the emit authority (only valid rows ship), and an `auto`-source with a FAILED gate is blocked via `auto_sources_blocked` (rc=1 aborts the build before emit); review-mode sources are quarantined. See `gates/__init__.py` docstring and the F11 note in AUDIT_2026-07-31.md._
