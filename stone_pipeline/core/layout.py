...
def write_sync_md(*, counts: dict, sources: list[str], products: dict[str, int],
    ...
    lines = [
        ...
        "- Upload combinations BEFORE any products. INITIAL load (once): `2_valid_combinations.csv`",
        "  -- the FULL set (large). DAILY: `2_valid_combinations_update.csv` -- ONLY the combinations",
        "  new since the last build (EMPTY when nothing changed), added into the existing set. If a",
        "  daily delta is ever skipped, re-upload the full file once to resync.",
