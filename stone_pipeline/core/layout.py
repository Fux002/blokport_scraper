...
def write_sync_md(*, counts: dict, sources: list[str], products: dict[str, int],
    ...
    lines = [
        ...
        "- Upload combinations BEFORE any products. `manifest.json` next to the files carries each",
        "  deliverable's sha256 + row count: load `2_valid_combinations.csv` (the FULL set) only when its",
        "  sha256 differs from the one last loaded; unchanged content is not re-uploaded. The old daily",
        "  `2_valid_combinations_update.csv` (new since the last BUILD, not since the last load) is retired.",
        "  (`2_valid_combinations_products_only.csv` -- combos for just the product sheet's variations",
        "  -- is a TESTING-phase convenience and drops off in production.)",
        "  One row per valid combination:",
