...
def build(existing_path: Path | None = None) -> Path:
    ...
    for r in rows:
        cat = category_for_key(r["Key"])
        # a variant gets the clean S3 image link if it already has an image OR a product references
        # it (its image is generated by the backfill queue) -- so a tile/block that gained a product
        # after its slab no longer stays blank. Imageless mirror tiles/blocks with NO product stay blank.
        if (r.get("Image") or "").strip() or r["Key"] in backed:
