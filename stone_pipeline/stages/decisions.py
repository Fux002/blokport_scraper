...
def load_decisions() -> Decisions:
    """EVERY operator decision, as the one object the stages consult (config.decisions_model.Decisions): mint
    and reject statements per (vendor, spelling), vendor bindings and vendor origins. Read once per produce."""
    return decisions_store.load_decisions()
