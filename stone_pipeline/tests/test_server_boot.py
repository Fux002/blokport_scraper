"""The config server boot sequence: restore config.db, seed, reconcile, restore the ledger, THEN the
two-level backfill (needs the ledger), artifacts, baseline, attribute vocab. A required restore that fails
raises before anything serves. Shutdown stops the periodic snapshot BEFORE the final save (the roll-time
...
def test_boot_runs_in_the_documented_order(monkeypatch, tmp_path):
    ...
    assert calls == ["restore_config", "seed", "reconcile", "restore_ledger", "backfill", "artifacts", "baseline", "attributes"]
    assert calls.index("backfill") > calls.index("restore_ledger")        # the backfill needs the ledger
